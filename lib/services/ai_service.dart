...
'''
      ...
      // Danh sách các model hỗ trợ trích xuất ảnh theo thứ tự ưu tiên
      ...
      for (final modelName in modelCandidates) {
        try {
          debugPrint('AI Service: Đang thử với model $modelName...');
          ...
          if (response.text != null && response.text!.trim().isNotEmpty) {
            debugPrint('AI Service: Thành công với model $modelName');
            return response.text!.trim();
          }
        } catch (e) {
          debugPrint('Lỗi khi gọi model $modelName: $e');
          errors.add(e);
        }
      }

      // Nếu tất cả các model đều thất bại, kiểm tra xem có bất kỳ lỗi 429/quota nào không
...
'''
      ...
      for (final modelName in modelCandidates) {
        try {
          ...
          debugPrint('Lỗi khi gọi model $modelName từ URL: $e');
          errors.add(e);
        }
      }
